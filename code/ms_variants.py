...
#   extra: ref == major (no loss of information) -> true for 99% of rs-pop combos
#
...
#
# fails1 = assumption1_checks.loc[(assumption1_checks == False)]
# fails1.to_frame().query("pop=='1000GENOMES:phase_3:EUR'").head()
# fails1.shape
#
# fails2 = assumption2_checks.loc[(assumption2_checks == False)]
# fails2.to_frame().query("pop=='1000GENOMES:phase_3:EUR'").head()
# fails2.shape
#
# ref_diff_major_fails = ref_diff_major_checks.loc[(ref_diff_major_checks == False)]
# ref_diff_major_fails.to_frame().query("pop=='1000GENOMES:phase_3:EUR'").head()
...
ld_partners = {}
# ld_partners2 = {}  # to continue loop midway, after valueError due to no 1KG EUR minor alleles for gene
vep_coding_mafs = {}
user_agents = get_user_agents_list()
...
# with open('/home/simon/OneDrive/KCL/DILI_GWAS_Molokhia/input_data/gwas-catalog-associations_ontology-annotated.tsv') as ebi_gwas_f:
#     ebi_gwas = pd.read_csv(ebi_gwas_f, sep='\t')
#
# ebi_gwas.columns = (ebi_gwas.columns
#                     .str.strip().str.lower().str.replace(' ', '_')
#                     .str.replace('(', '').str.replace(')', ''))
#
# ebi_gwas >>= mask(X.snps.isin(snps_1kg_eur))
#
# ebi_gwas.mapped_trait.value_counts().to_csv('gwas_traits.tsv', sep='\t')
